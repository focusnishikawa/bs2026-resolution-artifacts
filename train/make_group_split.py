...
def group_of(rel_path, species_key):
...
def report(rows):
    ...
    for s in SPLITS:
        for r in rows[s]:
            where[group_of(r["rel_path"], r["species_key"])].add(s)
    ...
    for s in SPLITS:
        n = sum(1 for r in rows[s]
                if group_of(r["rel_path"], r["species_key"]) in span)
...
def build(rows, out_dir):
    ...
    for s in SPLITS:
        for r in rows[s]:
            g = group_of(r["rel_path"], r["species_key"])
    ...
    for s in SPLITS:
        for r in out[s]:
            where[group_of(r["rel_path"], r["species_key"])].add(s)
    bad = [g for g, v in where.items() if len(v) > 1]
    print("\n=== 検証 ===")
