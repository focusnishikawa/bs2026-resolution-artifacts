#!/usr/bin/env bash
# 査読指摘 A-3 (v2 規則) の続き: 条件 A の DINOv2-L を可能にするため dinov2_l_r224 を
# group split v2 で 30 シード追加学習し、条件 A を 16/16 で完備させる (hpciaiss1 側).
#
...
#    3 構成ともスキップしていた (v1 の 2026-09-06 に seed 42 の実ログで確認)。
#
# ⭐ 実行ロジック・待ち方・GPU 割当・タイムアウトは v1 版 (fill_group_condA_dinov2.sh) と
#    **完全に同一**である。違うのは分割・出力先・ログ名だけ (正規化 diff で検証済み)。
#
# ⚠️ 本線 (run_group_split_boundary_v2.sh) と評価穴埋め (fill_group_eval_v2.sh) の
...
# ⚠️ dinov2_l_r224 は境界で最も重い構成である。v1 の実測 (seed 45) は r64 344s /
#    r144 791s / r160 473s なので、r224 は 900-1200 秒とみておく。30 シード / 4 並列で約 2-3 時間。
#
# 起動 (通常は train/run_group_v2_all.sh から呼ばれる):
#   cd /work/gfsi/ufsi0002/bs2026-resolution
#   nohup setsid bash train/fill_group_condA_dinov2_v2.sh > logs/fill_condA_dinov2_v2.log 2>&1 &
set -u
...
FILL_DONE="$PROJ/logs/fill_group_eval_v2.done"
...
flush_batch() {
    ...
    for s in $batch; do
        SPLITS_DIR="$SPG" nohup python3 train/run_phase2.py \
            ...
            --models_dir "models_group_v2_s${s}" --gpus "$g" --num_workers 10 \
            --progress_name "g2c_dinov2_224_s${s}.json" \
            >> "logs/fill_condA_train_s${s}.log" 2>&1 &
        g=$((g+1))
    done
    wait
    for s in $batch; do
        if [ -s "$PROJ/models_group_v2_s${s}/dinov2_l_r224/best.pt" ]; then
...
for s in $SEEDS; do
    # 既にあるものは学習しない (冪等)
    [ -s "$PROJ/models_group_v2_s${s}/dinov2_l_r224/best.pt" ] && { nb=$((nb+1)); continue; }
    batch="$batch $s"; i=$((i+1))
    [ "$i" -ge 4 ] && flush_batch
done
flush_batch
...
n_ckpt=0
for s in $SEEDS; do
    [ -s "$PROJ/models_group_v2_s${s}/dinov2_l_r224/best.pt" ] && n_ckpt=$((n_ckpt+1))
done
...
for s in $SEEDS; do
    out="$PROJ/results/G2_condA_s${s}"
    [ -s "$PROJ/models_group_v2_s${s}/dinov2_l_r224/best.pt" ] || {
        echo "    [skip] s${s} (重みが無い)"; continue; }
    mkdir -p "$out"
    t0=$(date +%s)
    CUDA_VISIBLE_DEVICES=0 $APP python3 train/eval_sweep.py \
        --mode A --models dinov2_l --resolutions $V2L \
        --models_dir "$PROJ/models_group_v2_s${s}" --output_dir "$out" \
...
ok=0; short=""
for s in $SEEDS; do
    n=$(count_json "$PROJ/results/G2_condA_s${s}")
...
if [ "$ok" -ge 30 ]; then
    touch "$PROJ/logs/fill_condA_dinov2_v2.done"
    exit 0
fi
