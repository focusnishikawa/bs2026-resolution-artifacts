#!/usr/bin/env bash
# ============================================================================
# 条件 A の DINOv2-L 評価を、30 シードすべてそろうまで繰り返す (group split v2)
# ============================================================================
#
# ⚠️ 背景 (v1 の 2026-09-07 実測):
#    fill_group_condA_dinov2_v2.sh は 30 シードを **1 巡するだけ**で、足りなければ
...
#    → S15 が本線 (fill_group_eval_v2.sh) で見つけた UCX の Caught signal 11 と
...
#
# ⭐ 実行ロジック・待ち方・リトライ回数は v1 版 (retry_group_condA_dinov2.sh) と
#    **完全に同一**である。違うのは分割・出力先・ログ名だけ (正規化 diff で検証済み)。
#
# 使い方 (通常は train/run_group_v2_all.sh から呼ばれる):
#    cd /work/gfsi/ufsi0002/bs2026-resolution
#    setsid nohup bash train/retry_group_condA_dinov2_v2.sh \
#        > logs/fill_condA_retry_v2.log 2>&1 < /dev/null &
# ============================================================================
set -u
...
running_1st() {
    ps -u "$(id -un)" -o cmd= 2>/dev/null | grep -c '[f]ill_group_condA_dinov2_v2\.sh'
}

T0=$(date +%s)
echo "===== 条件 A DINOv2-L のリトライ 開始 $(date '+%F %T') ====="

# ---- 1. 1 巡目 (fill_group_condA_dinov2_v2.sh) の終了を待つ (最大 60 分) ----
#    GPU を取り合わないため。実行中のスクリプトには一切触らない。
w=0
while [ "$(running_1st)" -gt 0 ]; do
    [ "$w" -ge 60 ] && { echo "[abort] 60 分待っても 1 巡目が終わらない"; exit 1; }
    [ "$w" -eq 0 ] && echo "[wait] 1 巡目 (fill_group_condA_dinov2_v2.sh) の終了を待つ"
    sleep 60; w=$((w+1))
done
...
if [ "$ok" -ge 30 ]; then
    ...
    echo "  マーカー logs/fill_condA_dinov2_v2.done を立てた"
    exit 0
fi
