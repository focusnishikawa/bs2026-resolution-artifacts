#!/usr/bin/env bash
# ViT-L の配備構成 ONNX を **シードごと** に書き出す (HPC 側).
#
# 査読指摘 A-4「ViT-L 級も配備エンジン上の分類精度で統一する」への対応。
# 現状 ViT-L の配備精度は seed 42 の 1 本しかなく、論文の図表は 30 シードの
# **サーバ FP32** 平均を代用している。CNN 側は 30 シードの配備精度なので基準が
# 混ざっている。そこで seed 43-71 についても同じ経路で ONNX を作る。
#
# ⭐ 由来を seed 42 と揃えることが最重要である (取り違えると別のグラフを測る):
#   DINOv2-L: FP16 ONNX -> **ORT を通さず** 4 分割 -> p3 をさらに 2 分割
#             (ORT を通すと MYELIN 融合が効かず 2.5 倍遅くなる。train/prep_vitl_raw.sh 参照)
#   DINOv3-L: FP16 ONNX -> fold_if (If を畳む) -> 4 分割 -> パート単位 ORT 最適化
#             (パート単位 ORT を掛けないと定数エンジンになる)
#
# ⚠️ 入力 bin (inputs/real_*.fp16.bin) は**シードに依存しない**ので作らない。既存を使う。
# ⚠️ N=32 について: seed 42 だけは旧経路の static 系 ONNX から作られている。本スクリプトは
#    全 14 水準を raw 経路で揃える。融合構造が違うだけで計算は同じであり、これは
#    seed 42 を raw 経路でも作って精度が一致することで確認する (run_vitl_acc_seed.sh の検証)。
#
# usage:
#   bash train/prep_vitl_seed.sh 43                # 全 14 水準
#   bash train/prep_vitl_seed.sh 43 112            # 1 水準だけ (パイロット用)
set -u

SEED="${1:?usage: prep_vitl_seed.sh <seed> [res...]}"; shift
...
for R in $RES; do
    ...
    # 分割済みが揃っている水準だけ消す (途中で落ちたときに作り直せるようにする)。
...
#    チェーンがそのシードを飛ばし、fgpu0 側が 1 水準だけで完了扱いにしかけた。
