#!/usr/bin/env bash
# 査読指摘 A-3 (v2 規則): **動画 ID を含む群定義**での group split 再分割・再学習・再評価.
#
# ⛔ なぜ作り直すのか: A-3 で使った群規則 (v1) の RE_FRAME は接頭辞なし `frame_...jpg` しか
#    拾わず、`<動画ID8桁>__frame_...jpg` の 1,000 枚が **1 枚 1 群 (SINGLE)** になっていた。
#    すなわち同一動画の隣接フレームが train/val/test に散ったままである。
#    v2 規則 (make_data_manifest.py:group_v2 と同一) で分割し直して、選択が動くかを確かめる。
#      分割の生成: python3 train/make_group_split.py --rule v2 \
#                      --out /work/gfsi/ufsi0002/bs2026-raptor/data/splits_group_v2
#
# ⭐ 実行ロジック・待ち方・GPU 割当・タイムアウトは v1 版 (run_group_split_boundary.sh) と
#    **完全に同一**である。違うのは分割・出力先・ログ名だけ (正規化 diff で検証済み)。
#
# 全 84 構成 x 30 シードの再学習は前回実績 5.46 時間/シード = 約 7 日かかる。レビューは
# 「広い探索を探索的結果、group split で再実験した候補群を確認的結果と明示する」ことを
# 許容しているので、**選択が入れ替わりうる境界の 16 構成**に絞る (ユーザー判断 2026-09-05)。
#
#   ResNet50  N=32/48/64/80/96/112/128/160/176/208/224 … 選択表の 0.80-0.93 とその近傍
#   DINOv2-L  N=64/144/160                             … 0.95/0.96 と 10 ms 予算の最良
#   ViT-S/16  N=112/224                                … サーバ精度で選ぶと選ばれる構成
#
# ⚠️ 学習・評価とも **v1 の成果物 (models_group_s* / results/G_cond*) を上書きしない**
#    別ディレクトリへ書く: models_group_v2_s<seed>/ , results/G2_cond{A,B}_s<seed>/
...
#   nohup setsid bash train/run_group_split_boundary_v2.sh 42 43 ... > logs/group_boundary_v2_master.log 2>&1 &
set -u
...
[ -d "$SPG" ] || { echo "[abort] group split が無い: $SPG (先に train/make_group_split.py --rule v2)"; exit 1; }
[ "$#" -ge 1 ] || { echo "usage: bash train/run_group_split_boundary_v2.sh <seed>..."; exit 1; }
SEEDS="$*"
rm -f logs/group_boundary_v2.done
...
for seed in $SEEDS; do
    MD="${PROJ}/models_group_v2_s${seed}"
    ...
    for spec in "resnet50:$R50" "dinov2_l:$V2L" "vit_small:$VITS"; do
        ...
        SPLITS_DIR="$SPG" python3 train/run_phase2.py \
            ...
            --models_dir "models_group_v2_s${seed}" \
            --progress_name "g2b_${m}_s${seed}.json" \
            --gpus 0 1 2 3 --num_workers 10 \
            >> "logs/group_boundary_v2_s${seed}.log" 2>&1
        echo "[$(date '+%H:%M:%S')]   学習 ${m} rc=$?  完了 $(ls ${MD}/*/test_metrics.json 2>/dev/null | wc -l)"
    done

    # ---- 2. 条件 A / B の評価 (4 GPU 並列) ----
    for mode in A B; do
        OUT="${PROJ}/results/G2_cond${mode}_s${seed}"
        mkdir -p "$OUT"
        ...
        for spec in "resnet50:$R50" "dinov2_l:$V2L" "vit_small:$VITS"; do
            ...
            nohup setsid bash -c "
                CUDA_VISIBLE_DEVICES=${i} ${APP_GPU} python3 train/eval_sweep.py \
                    ...
                    > logs/g2b_ev_${mode}_${seed}_${m}.log 2>&1
                touch logs/g2b_ev_${mode}_${seed}_${m}.done" > /dev/null 2>&1 &
            i=$((i+1))
        done
        # 3 本そろうまで待つ
        w=0
        while [ "$(ls logs/g2b_ev_${mode}_${seed}_*.done 2>/dev/null | wc -l)" -lt 3 ]; do
            sleep 30; w=$((w+1))
            [ "$w" -gt 240 ] && { echo "[abort] 条件 ${mode} の評価が 2 時間で終わらない"; exit 1; }
        done
        rm -f logs/g2b_ev_${mode}_${seed}_*.done
        echo "[$(date '+%H:%M:%S')]   条件 ${mode} 評価完了 $(ls "$OUT" | grep -cE '\.json$')/16"
    done
done
echo "[$(date '+%F %T')] ===== 全 seed 完了 ====="
touch logs/group_boundary_v2.done
