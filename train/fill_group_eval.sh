#!/usr/bin/env bash
# 査読指摘 A-3: group split の**評価だけ**をやり直して穴を埋める (hpciaiss1 側).
#
# ⛔ 本線 train/run_group_split_boundary.sh は条件ごとに **3 プロセスを同時起動**しており、
...
#    ⭐ **学習は 16/16 すべて成功している** (models_group_s44/s45 に 16 モデル・
#    「Phase 2 完了 seed=45: 3 本 / 失敗 0 本」) ので、やり直すのは**評価だけ**でよい。
#
# ⭐ 回避策の本体は **逐次実行**である。models_group_s45 の vit_small_r112 を
...
# ⚠️ 条件 A の DINOv2-L は models_group_s<seed>/dinov2_l_r224 が無いと評価できない
#    (境界 16 構成に N=224 を入れていないため)。追加学習は別スクリプトが行うので
#    本スクリプトは条件 A の DINOv2-L に触らない。→ train/fill_group_condA_dinov2.sh
...
#   nohup setsid bash train/fill_group_eval.sh > logs/fill_group_eval.log 2>&1 &
set -u
...
GPU="${FILL_GPU:-3}"
MASTER_LOG="$PROJ/logs/group_boundary_master.log"
...
# count_json <dir> : G_cond*_s<NN> の中の構成 JSON を数える (summary.json は数えない)
count_json() {
...
eval_one() {
    ...
    local out="$PROJ/results/G_cond${mode}_s${seed}"
    mkdir -p "$out"
    local t0; t0=$(date +%s)
    CUDA_VISIBLE_DEVICES="$GPU" $APP python3 train/eval_sweep.py \
        --mode "$mode" --models "$m" --resolutions $rs \
        --models_dir "$PROJ/models_group_s${seed}" --output_dir "$out" \
...
while true; do
    ...
    for s in $SEEDS; do
        ...
        [ "$s" = "$cur" ] && continue
        MD="$PROJ/models_group_s${s}"
        nm=$(ls "$MD" 2>/dev/null | wc -l)
        [ "$nm" -ge 16 ] || continue      # 学習がまだ済んでいない seed は飛ばす

        nb=$(count_json "$PROJ/results/G_condB_s${s}")
        na=$(count_json "$PROJ/results/G_condA_s${s}")
    ...
    if [ "$okA" -ge 30 ] && [ "$okB" -ge 30 ]; then
        ...
        touch "$PROJ/logs/fill_group_eval.done"
        exit 0
    fi
