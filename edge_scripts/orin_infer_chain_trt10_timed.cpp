...
int main(int argc, char** argv) {
  ...
  // ---- 査読指摘 A-6b: 合算時間モデルでなく**連続実行の wall-clock** を測る ----
  //  1 枚 = H2D コピー -> 全段の enqueue -> D2H コピー -> 同期 まで。
  //  ⚠️ CSV 整形は配備時のコストではないので計時に含めない。
  const char* wenv = getenv("WARMUP");
  const int warmup = wenv ? atoi(wenv) : 200;
  ...
  for (int i = 0; i < n; i++) {
    ...
    cudaStreamSynchronize(stream);
    {
      ...
      lat.push_back(std::chrono::duration<double, std::milli>(_t1 - _t0).count());
    }
  ...
  // ---- 統計 (ウォームアップ分を捨てる) ----
  {
    std::vector<double> w(lat.begin() + std::min((size_t)warmup, lat.size()), lat.end());
    if (!w.empty()) {
      std::vector<double> s = w;
      std::sort(s.begin(), s.end());
      double sum = 0; for (double x : w) sum += x;
      double mean = sum / w.size();
      double var = 0; for (double x : w) var += (x - mean) * (x - mean);
      var /= w.size();
      auto q = [&](double p) {
        double idx = p * (s.size() - 1);
        size_t lo = (size_t)idx; size_t hi = std::min(lo + 1, s.size() - 1);
        return s[lo] + (idx - lo) * (s[hi] - s[lo]);
      };
      fprintf(stderr, "[wall] n=%zu warmup=%d mean=%.4f sd=%.4f min=%.4f p50=%.4f "
              "p90=%.4f p95=%.4f p99=%.4f max=%.4f ms\n",
              w.size(), warmup, mean, sqrt(var), s.front(), q(0.50), q(0.90), q(0.95),
              q(0.99), s.back());
      const char* tout = getenv("TIMES_OUT");
      if (tout) {
        FILE* ft = fopen(tout, "w");
        if (ft) { for (double x : w) fprintf(ft, "%.6f\n", x); fclose(ft);
                  fprintf(stderr, "[wall] 生の計時を %s へ書いた\n", tout); }
      }
    }
  }
