#!/usr/bin/env bash
# ViT-S/16 の 14 構成を TensorRT 10.3 の **FP32** エンジンで測定する。電力モードごとに実行する。
#
# ⭐ measure_trt10_mode.sh (FP16) の FP32 変種である。**測定プロトコルは一切変えていない**。
#    変えたのは対象モデル (vit_small のみ)・エンジン置き場 (engines_trt10_fp32/)・
#    出力先 (results_trt10_fp32_*/) の 3 点だけで、これにより FP16 の実測値と
#    「精度モードだけが違う」比較ができる。
#
# ⭐ 測定プロトコル (FP16 版と同一):
...
#
# ⭐ 消費電力も同時記録する (VDD_IN / VDD_CPU_GPU_CV / VDD_SOC)。
#
# 使い方 (bash で起動すること):
#   bash measure_trt10_fp32_mode.sh maxn     # MAXN_SUPER (ID=2)
#   bash measure_trt10_fp32_mode.sh 15w      # 15W        (ID=0)
#
# 起動例:
#   nohup setsid bash measure_trt10_fp32_mode.sh maxn > logs/meas_trt10_fp32_maxn.log 2>&1 < /dev/null &
...
OUT="results_trt10_fp32_${MODE}"
TRTEXEC=/usr/src/tensorrt/bin/trtexec
ENG_DIR=engines_trt10_fp32
MODELS="vit_small"
...
mkdir -p "$OUT" logs
rm -f "$E/logs/meas_trt10_fp32_${MODE}.done"
...
for m in $MODELS; do
  for r in $RES; do
    ...
    t0=$(date +%s)
    # 一時ログは FP16 版と衝突しないよう fp32 を名前に入れる (中身の扱いは同じ)
    pwrlog="/tmp/pwr_fp32_${MODE}_${tag}.log"
    ...
    for rep in $(seq 1 "$REPS"); do
        replog="/tmp/rep_fp32_${MODE}_${tag}.log"
