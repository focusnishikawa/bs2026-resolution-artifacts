#!/usr/bin/env bash
# ViT-L の配備エンジンで **validation** の分類精度を測る (fgpu0 側・1 シード分).
#
# 2026-09-08 の再レビュー指摘 A-5 への対応。既存の run_vitl_acc_seed.sh は **test だけ**を
# 測っており、**構成選択に使う validation は ViT-L 級のみサーバ FP32 の代用**だった。
# 「配備エンジンで選び、配備エンジンで報告する」を全モデルで成立させるために val を測る。
#
# ⭐ 本スクリプトは run_vitl_acc_seed.sh の**分割違い**であり、演算精度・分割・由来は
#    一切変えていない (変えると test と比べられなくなる):
#   DINOv2-L 5 段: p0/p1/p2/p3s0 は FP16、**p3s1 のみ FP32**
#   DINOv3-L 4 段: **全段 FP32**
#
# ⚠️ エンジンは test と同一 (グラフも重みも同じ)。違うのは**入力 bin と枚数だけ**である。
#    ONNX は test 測定後に消してあるので、HPC 側 prep_vitl_seed.sh で作り直す必要がある。
# ⚠️ レイテンシは測らない (シードによらず同一)。ここで要るのは task accuracy だけ。
#
# usage:
#   bash run_vitl_acc_seed_val.sh 43            # 全 14 水準
#   bash run_vitl_acc_seed_val.sh 43 112        # 1 水準だけ (パイロット)
#   KEEP=1 bash run_vitl_acc_seed_val.sh 43     # エンジンと ONNX を消さない
set -u

SEED="${1:?usage: run_vitl_acc_seed_val.sh <seed> [res...]}"; shift
...
ENG="$S/engines_val"
OUT="$E/preds_vitl_30seed_val/s${SEED}"
IN="$E/inputs_val_vitl"
TRTEXEC=/usr/src/tensorrt/bin/trtexec
CHAIN=./orin_infer_chain_trt10
NIMG=1876                     # ⚠️ val は 1,876 枚 (test は 1,882 枚)

[ -x "$CHAIN" ] || { echo "[abort] $CHAIN が無い"; exit 1; }
[ -d "$S" ] || { echo "[abort] ONNX が無い: $S (HPC 側の prep_vitl_seed.sh を先に)"; exit 1; }
[ -s "$IN/real_dinov2_l_r224.fp16.bin" ] || {
    echo "[abort] val 入力 bin が無い: $IN (HPC 側の prep_inputs_vitl_val.sh を先に)"; exit 1; }
mkdir -p "$ENG" "$OUT" logs/vitl_val_s${SEED}
...
build() {
    ...
    if timeout 3600 "$TRTEXEC" --onnx="$onnx" --saveEngine="$eng" $prec \
            > "logs/vitl_val_s${SEED}/b_${name}.log" 2>&1; then
        printf "    [OK] %-28s %3ds %5d MB %s\n" "$name" "$(( $(date +%s)-t0 ))" \
               "$(( $(stat -c %s "$eng") / 1000000 ))" "${prec:-FP32}"
        nb=$((nb+1)); return 0
    fi
    echo "    [FAILED] $name"; grep -aE "\[E\]|Killed|out of memory" "logs/vitl_val_s${SEED}/b_${name}.log" | tail -2
...
run_chain() {
    ...
    if [ -s "$csv" ] && [ "$(wc -l < "$csv")" -eq $((NIMG+1)) ]; then
        n_skip=$((n_skip+1)); return; fi
    local t0; t0=$(date +%s)
    "$CHAIN" "$csv" "$NIMG" "$bin" "$@" > "logs/vitl_val_s${SEED}/i_${label}.log" 2>&1
    ...
    [ "$d" -lt $((NIMG/20)) ] && { n_const=$((n_const+1)); echo "      ⚠ 相異なる出力が 5% 未満。無効とみなす"; }
}

T0=$(date +%s)
echo "===== ViT-L 配備精度 (val) seed ${SEED} 開始 $(date '+%F %T') ====="
echo "  水準: $RES / 枚数: $NIMG / 入力: $IN"
...
for R in $RES; do
    ...
    [ "$v2ok" = "1" ] && run_chain "$OUT/dinov2_l_r${R}.csv" "$IN/real_dinov2_l_r${R}.fp16.bin" \
    ...
    [ "$v3ok" = "1" ] && run_chain "$OUT/dinov3_l_r${R}.csv" "$IN/real_dinov3_l_r${R}.fp16.bin" \
        "dinov3_l_r${R}" "$ENG"/dinov3_l_r${R}_p{0,1,2,3}.engine

    # 水準ごとにエンジンを捨てる (1 水準で約 4 GB)
    if [ "${KEEP:-0}" != "1" ]; then
        rm -f "$ENG"/dinov2_l_r${R}_*.engine "$ENG"/dinov3_l_r${R}_*.engine
    fi
done

echo "===== seed ${SEED} (val) 完了 $(date '+%F %T') ====="
...
if [ "$nf" -eq 0 ] && [ "$n_ng" -eq 0 ] && [ "$n_const" -eq 0 ] && [ "$have" -eq 28 ]; then
    touch "$E/logs/vitl_acc_val_s${SEED}.done"
    # ONNX も落とす (1 シード 27 GB)。CSV は残る
    [ "${KEEP:-0}" != "1" ] && rm -rf "$S/onnx_split_raw" "$S/onnx_split_raw_p3" "$S/onnx_split" "$S/onnx" "$ENG"
    exit 0
fi
rm -f "$E/logs/vitl_acc_val_s${SEED}.done"
