#!/usr/bin/env bash
# ViT-S/16 の 30 シード「配備精度」を Orin 実機の **FP32 エンジン**で測る — test と val の両方。
#
# run_30seed_deploy_acc_v2.sh (FP16) の FP32 変種である。**設計はそのまま**で、違いは 5 点だけ。
#
#   1. 対象モデルは vit_small のみ (FP16 で崩れるのがこのモデルなので、比較対象もこれだけでよい)
#   2. ビルドで **--fp16 を付けない** (= FP32 エンジン)
#   3. 出力先が preds_30seed_fp32/ (test) と preds_30seed_fp32_val/ (val)
#   4. **既定のシード範囲は 42-71 の 30 シード** (v2 は 43-71 だった。FP32 は seed42 も未測定である)
#   5. 一時エンジン置き場・ログ接頭辞・完了マーカーを fp32 用に分けた
#
# ⭐ **1 本のエンジンで test と val を続けて測る** (v2 の要点をそのまま踏襲)。
#   ビルドが 1.8 分/構成に対し推論は 5 秒/構成で、時間の 96% はビルドである。
#   val を別ジョブにすると全エンジンを作り直すことになるので、同じエンジンで続けて測る。
#
# 冪等性 (v2 と同じ):
...
#
# 起動: nohup setsid bash run_30seed_deploy_acc_fp32.sh > logs/deploy_acc_30seed_fp32_master.log 2>&1 < /dev/null &
# 監視: tail -f logs/deploy_acc_30seed_fp32_master.log
# 完了: logs/deploy_acc_30seed_fp32.done
set -u
...
# ⭐ 試験用に絞れる (例: RES="16 112" bash ... 42 42)
MODELS="${MODELS:-vit_small}"
RES="${RES:-16 32 48 64 80 96 112 128 144 160 176 192 208 224}"
S0="${1:-42}"; S1="${2:-71}"
ONNX_ROOT=onnx_seeds
TMP_ENG=engines_30seed_fp32_tmp

# 1 シードあたりの構成数 (v2 は vit_small を含む 4 モデル x 14 で 56 固定だった。
# 本版は 1 モデルなので、進捗表示・スキップ計上がずれないよう実数から求める)
n_models=$(echo $MODELS | wc -w | tr -d ' ')
n_res=$(echo $RES | wc -w | tr -d ' ')
N_CFG=$(( n_models * n_res ))
PROG_MOD=$n_res; [ "$PROG_MOD" -gt 0 ] || PROG_MOD=1

# 分割 (test / val) の定義。DO_SPLITS で片方だけに絞れる。
DO_SPLITS="${DO_SPLITS:-test val}"
IN_test=inputs;      OUT_test=preds_30seed_fp32;     N_test=1882
IN_val=inputs_val;   OUT_val=preds_30seed_fp32_val;  N_val=1876

mkdir -p "$TMP_ENG" logs
rm -f logs/deploy_acc_30seed_fp32.done
...
echo "===== 30 シード配備精度 FP32 (test+val) seed ${S0}-${S1} 開始 $(date '+%F %T') ====="
echo "      モデル: ${MODELS} / 分割: ${DO_SPLITS} / 1 シード ${N_CFG} 構成"
...
for s in $(seq "$S0" "$S1"); do
    ...
    if [ "$n_need" -eq 0 ]; then
        ...
        tot_skip_cfg=$((tot_skip_cfg+N_CFG)); continue
    fi
    ...
    for m in $MODELS; do
        for r in $RES; do
            ...
            # --- ビルド (test と val で 1 本を共用する) ---
            # ⭐ --fp16 を付けない。これが FP32 エンジンである。
            t0=$(date +%s)
            if ! timeout 1800 "$TRTEXEC" --onnx="$onnx" --saveEngine="$eng" \
                    > "logs/b30f_s${s}_${m}_r${r}.log" 2>&1; then
            ...
            for sp in $need; do
                ...
                eval "nn=\$N_$sp"
                lg="logs/i30f_s${s}_${m}_r${r}.log"
                [ "$sp" = "val" ] && lg="logs/i30fv_s${s}_${m}_r${r}.log"
                "$RES1" "$eng" "$b" "$nn" "$r" "$o" > "$lg" 2>&1
                rc=$?
                d=$(distinct "$o")
                # ⭐ 壊れたエンジンは入力によらず定数を返し、計算が消えるぶん速く見える。
                #    latency では気づけないので、ここで相異なる出力を必ず数える
                #    (FP32 では起きにくいが、FP16 との比較のために同じ検査を残す)。
                if [ "$rc" -ne 0 ]; then
            ...
            # エンジンは 1 本ごとに消す (ためるとディスクを食う)
            rm -f "$eng"
            [ $(( (s_build + s_ng) % PROG_MOD )) -eq 0 ] && \
                printf "  [%s] s%-3s 進捗 build %2d/%d (test %2d / val %2d)  最後=%s_r%s build %ds\n" \
                       "$(date '+%H:%M:%S')" "$s" "$((s_build+s_ng))" "$N_CFG" "$s_it" "$s_iv" "$m" "$r" "$tb"
        done
    done
...
touch logs/deploy_acc_30seed_fp32.done
exit 0
